...
class NewsGroup20Processor(DataProcessor):
    """Processor for the WNLI data set (GLUE version)."""
    ...
    def _create_examples(self, set_type):
        """Creates examples for the training and dev sets."""
        dataset =  fetch_20newsgroups(subset=set_type, shuffle=True,  categories=self.get_labels())
        examples = []
        for i, (item, target) in enumerate(zip(dataset.data, dataset.target)):
            guid = "%s-%s" % (set_type, i)
            text_a = item
            label = self.get_labels()[target]
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label))
        return examples
...
total_ex = 0
def run_one_step(ex_index, example, label_map, tokenizer):
    tokens_a = tokenizer.tokenize(example.text_a)

    if ex_index % 1000 == 0:
        logger.info("Writing example {} of {}".format(ex_index, total_ex))

    if len(tokens_a) >= tokenizer.max_len:
        tokens_a = tokens_a[:tokenizer.max_len ]

    input_ids = tokenizer.convert_tokens_to_ids(tokens_a)
    input_mask = [1] * len(input_ids)

    padding = [0] * (tokenizer.max_len - len(input_ids))
    input_ids += padding
    input_mask += padding
    ...
    if ex_index < 5:
        ...
        logger.info("tokens: %s" % " ".join(
                [str(x) for x in tokens_a[:128]]))
        logger.info("input_ids: %s" % " ".join([str(x) for x in input_ids[:128]]))
        logger.info("label: {} (id = {})".format(example.label, label_id))

    return InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=None, label_id=label_id)


#https://github.com/mireshghallah/20Newsgroups-Pytorch/blob/master/newsgroup-pytorch-git.ipynb
from copy import deepcopy
import itertools
def convert_examples_to_features(examples, label_list, tokenizer, max_len):
    ...
    results = map_fn(run_one_step, range(len(examples)), examples, itertools.repeat(label_map), itertools.repeat(tokenizer))
    return results
...
def compute_metrics(task_name, preds, labels):
    ...
    if task_name == "cola":
        ...
        return {"acc": simple_accuracy(preds, labels)}
    elif task_name == "patent36":
        return precision_recall_f1(preds, labels)
    elif task_name == "patent36sub":
        return precision_recall_f1_multi(preds, labels, [0,1,2,3,4,5])
