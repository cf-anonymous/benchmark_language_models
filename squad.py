...
from model import ReformerConfig16k, ReformerConfig512, ReformerForQuestionAnswering
...
MODEL_CLASSES = {
    ...
    "reformer512": (ReformerConfig512, ReformerForQuestionAnswering, BertTokenizer),
    "reformer16k": (ReformerConfig16k, ReformerForQuestionAnswering, BertTokenizer),
...
def main():
    ...
    if "v2" in args.predict_file and not args.version_2_with_negative:
        args.version_2_with_negative = True
        logger.warning("WARNING - You are running squadv2 without negatives")
    ...
    new_train_data = []
    val_data = []
    rand = torch.randperm(train_dataset.tensors[0].shape[0])
    val_size = int(train_dataset.tensors[0].shape[0] * args.val_split)

    for train_datax in train_dataset.tensors:
        train_data_cur = train_datax[rand]
        val_data_cur = train_data_cur[:val_size]
        train_data_cur = train_data_cur[val_size:]

        new_train_data.append(train_data_cur)
        val_data.append(val_data_cur)


    train_dataset = TensorDataset(*tuple(new_train_data))
    val_dataset = TensorDataset(*tuple(val_data))
